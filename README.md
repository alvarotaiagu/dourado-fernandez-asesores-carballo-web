...
Sitio estático sin build: `index.html` + `css/style.css` + `js/main.js`. GSAP 3.12 + ScrollTrigger y Lenis 1.1 desde CDN; Fraunces, Poppins e Inter desde Google Fonts. Sin canvas, sin shaders, sin imágenes de fondo: la retícula de papel contable es CSS.
...
- **Fraunces** (opsz 144, SOFT 0, WONK 0) para titulares y cifras; **Inter** para cuerpo. Las cifras clave van en serif, grandes, con `font-variant-numeric: tabular-nums` y una **regla fina debajo** que se dibuja al entrar (`.regla-cifra`, `scaleX 0 → 1`). **Excepción, 2026-09-21**: el wordmark literal «Dourado & Fernández» (cabecera, hero, pie, cortina) va en **Poppins Medium** (`--sans-marca`), no en Fraunces — el cliente señaló que la tipografía de su tarjeta real es una sans geométrica, no la serif editorial que lleva el resto del sitio. Es una excepción deliberada: el nombre de marca usa su tipografía real, los titulares de contenido siguen en la serif del concepto "Balance".
...
**Logo real, recibido el 2026-09-21** en la tarjeta física del cliente (ya no es la recreación provisional de las letras de acrílico sobre la pared verde, que resultó no coincidir ni en color ni en forma). El monograma es «D&F»: una D y una F de palo grueso, y entre ambas una cinta vertical de bordes cóncavos (más ancha arriba y abajo, como un lazo o banderola) que aloja el «&» recortado en el color de fondo — la misma tinta cubre D, cinta y F, y solo el ampersand cambia de color. `scripts/generate_brand.py` lo reconstruye a mano (no es un trazado literal de la foto: coordenadas propias para el monograma, y el glifo "&" de **Fraunces** vía fontTools para el recorte — el ampersand del monograma es un dibujo de marca, más caligráfico que el de la tarjeta) junto con el wordmark **«dourado & fernández»** en minúsculas y la marquilla **«ASESORÍA · CORREDORÍA»**, ambos en **Poppins** (Medium/SemiBold) convertidos a trazados — corregido el mismo día de una primera versión en Fraunces que el cliente señaló que no coincidía con la sans geométrica real de la tarjeta. Sin degradados metalizados (los oro/plata de antes no existen en la tarjeta real): tinta plana, con dos variantes de color intercambiables igual que las dos caras de la tarjeta.
