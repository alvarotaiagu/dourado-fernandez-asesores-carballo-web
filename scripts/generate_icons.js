...
(async () => {
  ...
  for (const size of TAMANOS) {
    ...
    await page.setContent(
      `<body style="margin:0;width:${size}px;height:${size}px;background:#7E1E22">` +
  ...
  /* Open Graph 1200x630: fondo rojo de marca con el wordmark claro y los datos reales */
  const og = await browser.newPage({ viewport: { width: 1200, height: 630 } });
  await og.setContent(`<!doctype html><html><head>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@500&display=swap" rel="stylesheet">
    </head><body style="margin:0;width:1200px;height:630px;background:#5C1114;display:flex;flex-direction:column;
      align-items:center;justify-content:center;gap:44px;
      background-image:repeating-linear-gradient(to bottom, transparent 0 35px, rgba(245,241,230,.06) 35px 36px)">
      ...
      <p style="margin:0;font-family:Inter,sans-serif;font-weight:500;font-size:19px;letter-spacing:.3em;
        text-transform:uppercase;color:#D9D2C2">Carballo · 981 70 27 60 · 4,2 ★ Google</p>
    </body></html>`);
